...
* @details   Range is between 0mA and 200mA, internally set in steps of 10mA with input values being truncated down 
*            to the next lower value. No range checking is performed
...
/*!
* @brief     Sets the number of samples taken per second and the number of samples averaged to make a reading
* @details   each reading takes only 300microseconds and the default period for a measurement is 100ms.
* @param[in] sample Samples to take per second
* @param[in] avg Averages to take per reading (0-128, rounded internally to nearest correct value)
*/
...
* @details   Since we always send a request for another reading after retrieving the previous results we just need 
*            to wait for a result to come back. 
...
{
  return readByte(VCNL4010_INTERRUPT_STATUS_REG) & B0001111; // get the register contents and mask unused bits
} // of method getInterrupt()

/*!
* @brief     clearInterrupt() overwrites the 4 bits denoting which, if any, interrupts have been triggered
* @details   Bit 3 - proximity interrupt\n
*            Bit 2 - ambient light interrupt\n
*            Bit 1 - low threshold interrupt\n
*            Bit 0 - high threshold interrupt\n
...
*/
void VCNL4010::clearInterrupt(const uint8_t intVal)
{
  writeByte(VCNL4010_INTERRUPT_STATUS_REG,(readByte(VCNL4010_INTERRUPT_STATUS_REG)&0xF0)|(~intVal&0xF));
} // of method clearInterrupt()

/*!
* @brief     sets the interrupts used
* @details   The count is the number of consecutive readings needed in order to trigger an interrupt for threshold 
*            low or high being exceeded. Then a boolean to indicate an interrupt on Proximity reading ready, then 
*            the same for an ALS reading to be ready. The next two values indicate whether to trigger an interrupt 
*            on threshold low or high value being exceeded for Proximity and ALS. Since only one can be used the ALS
*            is chosen when both are set. Finally the low and high threshold values themselves
* @param[in] count
* @param[in] ProxReady
* @param[in] ALSReady
...
void VCNL4010::setInterrupt(const uint8_t  count, const bool ProxReady, const bool ALSReady,
                            const bool ProxThreshold, const bool ALSThreshold, const uint16_t lowThreshold,
                            const uint16_t highThreshold ) 
{
  uint8_t registerValue = 0; // Default to 1 count
  if (count>=128)       registerValue = B111;
...
void VCNL4010::setAmbientContinuous(const bool ContinuousMode)
{
  ...
  {                                                       // otherwise if proximity is still
