/*!
@file WakeOnInterrupt.ino

@section WakeOnInterrupt_intro_section Description

Example program for using the VCNL4010 library which allows the Arduino to use the VCNL4010 Fully
Integrated Proximity and Ambient Light Sensor with Infrared Emitter using the built-in interrupt
functionality.

The example will use the proximity sensor and put the Arduino to sleep in low-energy mode until the
VCN4010 detects a movement in the sensor. The movement is detected using the low and high threshold
value settings, are set to 10% below and 10% above the startup values to avoid false triggering.

The interrupt makes the Arduino wake up and display a message, then it goes back to sleep.

The WAKE_UP_PIN is connected to the INT pinout on the VCNL4010 and is pulled low when the interrupt
is raised on the VCNL4010. This pin needs to be attached to one of digital pins to interrupt, which
are just a few pins depending upon which processor is used. Check
https://www.arduino.cc/en/Reference/attachInterrupt to see the pins which may be used.

This program was originally developed on an Arduino Micro, which poses difficulties because of the
way that the USB serial port is implemented. This needs to be turned off before sleeping and back on
after sleeping and the specific code for this is put into a conditional compile block.

@section WakeOnInterrupt_license GNU General Public License v3.0
...
@section WakeOnInterrupt_author Author
...
void sleepVector(void) {
  /*!
   * @brief     Interrupt vector to jump to when the VCNL4010 triggers a pin change interrupt
   * @details   Nothing is done here apart from disabling the interrupt to prevent a race condition
                where the program jumps here constantly
   */
...
void setup() {
  ...
  */
  ...
  Serial.print(ProximityVal);  // Display reading
  Serial.print(", low = ");
  Serial.print(lowThreshold);  // Display low threshold value
  Serial.print(", high = ");
  Serial.print(highThreshold);  // Display high threshold value
  Serial.println();
...
void loop() {
  ...
  */
  ...
  attachInterrupt(digitalPinToInterrupt(WAKE_UP_PIN), sleepVector,
                  LOW);                 // jump to vector when pin goes LOW
